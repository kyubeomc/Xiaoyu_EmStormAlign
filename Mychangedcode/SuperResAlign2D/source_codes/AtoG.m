function [ParG,code] = AtoG(ParA) % angle to geometric parameters
...
% ParA = ParA/norm(ParA);
if (abs(ParA(1)-ParA(3)) > tolerance1) % if A and C is not same
...
Q = [c s; -s c]; % rotation matrix counter clockwise
M = [ParA(1)  ParA(2)/2;  ParA(2)/2  ParA(3)];   
% think this might be the conic section  Marix notation   
% [ https://en.wikipedia.org/wiki/Matrix_representation_of_conic_sections]

D = Q*M*Q';
  % not sure this is right Q' is rotation check the website 
  % maybe Q is Q * [x,y] 
N = Q*[ParA(4); ParA(5)]; % this was normally be [D, E] * (x ; y)
O = ParA(6);

if (D(1,1) < 0) && (D(2,2) < 0) % if A & C is - | | 
    D = -D;
    N = -N;
    O = -O;
end
